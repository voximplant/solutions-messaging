...
import { Conversation, ConversationsState, User } from '@/types/src/conversations';
...
const actionsConversations: ActionTree<ConversationsState, any> = {
  ...
  getCurrentConversation: (context: any, chatUuid: number | string) => {
    if (chatUuid === context.state.currentConversationId) {
      ...
      if (!context.getters.currentConversationHistory && context.getters.currentConversation) {
  ...
  onConversationCreated: ({ state, commit }, e: any) => {
    ...
    if (e.conversation.customData.type === 'direct') {
      const participant = e.conversation.participants.find((p: any) => p.userId !== state.currentUser.userId);
    ...
    if (e.initiator === state.currentUser.userId) {
      ...
      const newUsers = e.conversation.participants.filter((p: User) => !state.users.some((u: User) => u.userId === p.userId));

      if (newUsers.length) {
        const usersIds = newUsers.map((u: User) => u.userId);
        MessengerService.messenger.getUsersById(usersIds)
          .then((events: any) => {
            commit('updateUsers', events.map((e: any) => e.user));
          })
          .catch(logError);
      }
    }
    ...
    const otherParticipantIds = e.conversation.participants.map((p: User) => p.userId).filter((id: number) => id !== state.currentUser.userId);
  ...
  createConversation: ({ state, dispatch }, newChat) => {
    try {
      ...
      } else { logError(`Conversation type ${newChat.type} is unknown`); }
    } catch (e) { logError(e); }
  ...
  onConversationEdited: ({ state, commit }, e: any) => {
    ...
    if (!state.conversations.some((c:Conversation) => c.uuid === e.conversation.uuid)) {
    ...
    } else if (!e.conversation.participants.some((p: User) => p.userId === state.currentUser.userId)) {
      log(`Left conversation ${e.conversation.title}`, e);

      // Update conversation list
      // TODO to mutation
      state.conversations = state.conversations.filter((c: any) => c.uuid !== e.conversation.uuid);
  ...
  addNewAdmins: (context, userIds) => {
    MessengerService.get().addAdmins(context.getters.currentConversation, userIds)
      .then((evt: any) => {
  ...
  editChatPermissions: (context, permissions) => {
    const allUsersIds = context.getters.possibleChatAdmins.map((u: User) => u.userId);
    MessengerService.get().editPermissions(context.getters.currentConversation, permissions, allUsersIds)
      .then((evt: any) => {
  ...
  onNotifyTyping: (context, evt) => {
    if (evt.initiator !== context.state.currentUser.userId && evt.conversation === context.state.currentConversationId) {
      ...
      const userName = context.state.users.find((u: User) => u.userId === evt.initiator).displayName;
