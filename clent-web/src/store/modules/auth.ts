...
const actions: ActionTree<AuthState, any> = {
  login({ commit, state, dispatch }, loginForm: LoginForm) {
    ...
    voxService.onLogin(fullLoginForm, state.accessToken)
      .then((data: any) => {
    ...
    voxService.onLogin(fullLoginForm, state.accessToken)
      .then((data: any) => {
        dispatch('conversations/getInitialData', {}, { root: true }).then(() => {
          if (router.currentRoute.params.chatUuid) {
            dispatch('conversations/getCurrentConversation', router.currentRoute.params.chatUuid, { root: true });
          }
        });
  ...
  onError({state, commit, dispatch}: any, errorData: {error: any, loginForm: LoginForm}) {
    // @ts-ignore
  ...
  tryRefreshTokens({commit, state, dispatch}, loginForm: LoginForm) {
    voxService.refreshTokens(loginForm.password, state.refreshToken)
      .then((response: any) => {
        dispatch('loginSuccess', {user: loginForm.user, tokens: response.tokens});
      })
      .catch((error: any) => {
        dispatch('onError', { error, loginForm });
      });
  ...
  loginSuccess({state, commit, dispatch}, data) {
    ...
    state.rememberData ? storageService.setTokens(data.tokens, data.user) : storageService.removeTokens();

    dispatch('conversations/getInitialData', {}, { root: true });
    router.push('/');
...
const mutations: MutationTree<AuthState> = {
  ...
  updateAuthError: (state, newError: number) => {
    if (!!newError) {
      ...
      // @ts-ignore
      const errorMessage = ERROR_MESSAGES[newError] || 'Invalid username or password';
