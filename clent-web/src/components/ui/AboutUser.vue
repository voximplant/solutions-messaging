    ...
    v-dialog(
      ...
      v-card(class="pa-0")
        v-card-title
          v-row(class="pa-0")
            ...
            v-col(cols="6" class="about-user__name pa-0")
              span(class="headline") {{ user.displayName}}
            v-col(v-if="user.userId !== currentUser.userId" cols="4" class="about-user__name pa-0")
              v-btn(color="indigo darken-1" outlined @click="gotoDirect") Send message
        v-card-text
          v-container
            v-row(class="pa-0")
        ...
        v-card-actions(class="about-user__actions")
          ...
          v-btn(v-if="user.userId === currentUser.userId" color="red darken-1" text @click="editUserCustomData") Save
    ChooseAvatar(
...
export default class AboutUser extends Vue {
  ...
  public statusRules = [(v:string) => v.length <= 67 || 'Status must be not more than 67 characters'];
